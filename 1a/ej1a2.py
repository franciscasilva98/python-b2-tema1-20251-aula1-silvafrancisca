...
def create_event(name: str, datetime_start: datetime, timezone_str: str) -> Dict[str, str]:
    ...
    datetime_localized = timezone.localize(datetime_start)
    event = {"name": name,
            "datetime_start": datetime_localized,
...
def time_until_event(event: Dict[str, str]) -> timedelta:
    time_now = datetime.now(pytz.timezone(event["timezone"]))
    time_until_event = event["datetime_start"] - time_now
...
def find_next_event(events: List[Dict[str, str]]) -> Optional[Dict[str, str]]:
    ...
    future_events = [event for event in events if event["datetime_start"] > time_now]
